local B={active=false,projectiles={},feathers={},nests={},name='Le Merle noir'}
...
function B.reset(active)
    B.active=active; B.projectiles={}; B.feathers={}; B.nests={}; B.hp=12; B.maxHp=12; B.elapsed=0; B.shot=0.7; B.flash=0; B.defeated=false
    B.x=Arena.width/2; B.y=300; B.relocations=0
    if not active then return end
    for _,p in ipairs({{.32,180},{.68,180},{.25,350},{.75,350},{.4,445},{.6,445}}) do
        B.nests[#B.nests+1]={x=Arena.width*p[1],y=p[2],rx=34,ry=25}
    end
    B.spots={}
    for _,p in ipairs({{0.12,105},{0.32,100},{0.68,100},{0.88,105},{0.11,210},{0.89,210},{0.11,445},{0.28,510},{0.72,510},{0.89,445}}) do
        B.spots[#B.spots+1]={x=Arena.width*p[1],y=p[2]}
    end
    for i,spot in ipairs({1,4,7,10}) do local p=B.spots[spot]; B.feathers[i]={x=p.x,y=p.y,spot=spot,glow=0} end
end
function B.interval()
    return math.max(0.085,0.24-B.elapsed*0.0015-(1-B.hp/B.maxHp)*0.12)
end
function B.relocate(f)
    local occupied={}; for _,other in ipairs(B.feathers) do occupied[other.spot]=true end
    local candidates={}
    for i,p in ipairs(B.spots) do
        if not occupied[i] and (p.x-f.x)^2+(p.y-f.y)^2>10000 then candidates[#candidates+1]=i end
    end
    local choice=candidates[love.math.random(#candidates)]
    f.spot=choice; f.x=B.spots[choice].x; f.y=B.spots[choice].y; f.glow=0.35
    B.relocations=B.relocations+1
end
function B.fire()
    local angle=math.atan2(player.y+12-B.y,player.x+15-B.x)
    local speed=285+70*(1-B.hp/B.maxHp)
    -- The telegraphed opening becomes a rapid aimed stream, then three-way volleys.
    local angles={angle}
    if B.hp<=4 then angles={angle-0.15,angle,angle+0.15} end
...
function B.update(dt)
    ...
    for _,f in ipairs(B.feathers) do f.glow=math.max(0,f.glow-dt) end
    ...
    for i=#B.projectiles,1,-1 do
        ...
        for _=1,steps do
            ...
            -- A projectile must actually hit a feather. The player cannot damage the boss directly.
            for _,f in ipairs(B.feathers) do
                if not dead and (p.x-f.x)^2+(p.y-f.y)^2<21^2 then
                    B.hp=math.max(0,B.hp-1); B.flash=0.18; B.relocate(f); Audio.play('pick'); dead=true; break
                end
            end
        ...
        if B.hp==0 then
            B.defeated=true; B.projectiles={}; B.feathers={}
...
function B.drawGround()
    ...
    for _,f in ipairs(B.feathers) do
        g.setColor(1,0.81,0.35,0.4+f.glow); g.ellipse('line',f.x,f.y+15,24,9)
        g.setColor(1,1,1); Art.draw('feather',f.x,f.y+math.sin(larme_float_timer*3)*3,30)
    end
end
function B.draw()

...
function G.draw()
 local U=UI;U.panel(305,95,590,565);U.text('Graphismes',340,125,'heading')
 U.text('Choisis le rendu qui reste fluide sur ton écran.',340,174,'small',{.7,.8,.8})
 U.button('Qualité : '..({'Légère','Équilibrée','Élevée'})[G.quality],340,215,520,45,function() G.quality=G.quality%3+1;G.save() end)
 U.button('Lumières décoratives : '..(G.effects and 'activées' or 'désactivées'),340,280,520,45,function() G.effects=not G.effects;G.save() end)
 U.button('Synchronisation verticale : '..({[-1]='adaptative',[0]='désactivée',[1]='activée'})[G.vsync],340,345,520,45,function()
  G.vsync=G.vsync==0 and 1 or G.vsync==1 and -1 or 0;love.window.setVSync(G.vsync);G.save()
 end)
 U.button('Compteur FPS : '..(G.showFPS and 'visible' or 'masqué'),340,410,520,45,function() G.showFPS=not G.showFPS;G.save() end)
 U.button('Limite FPS : '..(G.limit==0 and 'sans limite' or tostring(G.limit)),340,475,520,45,function() G.limit=G.limit==120 and 60 or G.limit==60 and 0 or 120;G.save() end)
 U.text('En cas de ralentissements : qualité légère et lumières désactivées.\nLa lumière nécessaire au combat des abysses reste active.',340,534,'small',{.65,.77,.79},520)

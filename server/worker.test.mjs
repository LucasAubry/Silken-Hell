...
test('top ten uses each player best run and its associated skin',async()=>{
 ...
 assert.equal(scores.length,10);assert.equal(scores[0].skin,1);assert.equal(scores[1].name,'Player1');assert.equal(scores[1].skin,4);assert.equal(scores[9].name,'Player9');
});
test('all nicknames on one computer remain visible across stable pages',async()=>{
 ...
 for(let page=1;page<=3;page++) {
  ...
  assert.equal(data.total,23);assert.equal(data.page,page);assert.equal(data.hasMore,page<3);
  assert.equal(data.scores.length,page===3?3:10);rows.push(...data.scores);
 }
 ...
 for(const page of ['0','-1','1.5','no','1000001']) assert.equal((await call(env,'/v1/leaderboard?world=5&page='+page)).status,400);
});
...
test('Renaissance migration retains historical runs and scores',()=>{
 const db=new DatabaseSync(':memory:');
 for(const file of readdirSync(new URL('migrations/',import.meta.url)).sort().filter(n=>n<'0005')) db.exec(readFileSync(new URL('migrations/'+file,import.meta.url),'utf8'));

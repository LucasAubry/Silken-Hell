...
function T.run()
    Profile.unlocked=6
    Campaign.starts={{},{},{},{},{},{}}
    for _,w in ipairs({4,5,6}) do for n=1,10 do
        level(w,n)
        assert(not Arena.blocked(player.x,player.y,30,24),'Spawn nouveau monde libre')
        assert(#mobs>=4 and Realms.floor,'Rencontre et décor présents')
        ...
        if Campaign.carrier then
            ...
            for _,m in ipairs(mobs) do if m.type=='piege' then trap=m; break end end
    ...
    level(6,3); local cloud=Realms.clouds[1]
    player.x=cloud.x-15; player.y=cloud.y-12; assert(Hazards.speed()==.4)
    ...
    level(2,10); Wasp.phase='landed'; player.x=Wasp.x-15; player.y=Wasp.y-12
    ...
    assert(Worlds.next(2)==4 and not Worlds.canEnter(3),'Renaissance réservée')
    for _,w in ipairs({4,5,6}) do
        App.start(w)
        for n=1,10 do
    ...
    print('PASS release: 30 nouveaux niveaux, porteurs, pluie/nuages, guêpe sûre, bestiaire persistant, skins des scores, progression, UI et redimensionnement')
end
return T
